...
namespace Msh.WebApp.API.Admin.Hotels
{
	public partial class HotelApiController
	{
		...
		public async Task<IActionResult> RatePlansPublish(string hotelCode)
		{
			try
			{
		...
		public async Task<IActionResult> RatePlansArchive(string hotelCode, string archiveCode)
		{
			try
			{
				var userId = userService.GetUserId();
				...
				var result = await ratePlanRepository.Archive(hotelCode, archiveCode, userId);
				if (!result)
				{
